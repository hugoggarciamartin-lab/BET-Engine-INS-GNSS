...
def generate_allan_variance_data():
    ...
    w_nominal = np.array([0.0, Omega_e * np.cos(phi), Omega_e * np.sin(phi)], dtype=np.float64)
    ...
    output_filename = 'lab_allan_variance_data.csv'
    df.to_csv(output_filename, index=False, float_format='%.8f')
    print(f"Dataset successfully written to {output_filename}")
