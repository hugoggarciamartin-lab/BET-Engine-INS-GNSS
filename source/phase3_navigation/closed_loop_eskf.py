...
def initialize_eskf_mem(n_epo: int) -> dict:
    ...
    return {
        "x_nom": np.zeros((n_epo, 10), dtype=np.float64),
...
def inject_error_state(x_nom: np.ndarray, delta_x: np.ndarray) -> None:
    ...
    # Position Injection (Lat, Lon, Alt)
    # delta_x[0:3] is delta_r = [delta_r_E, delta_r_N, delta_r_U] (ENU) (rad)
    # x_nom[0:3] = [lat_rad, lon_rad, alt_m] (Geodetics)
...
def main():
    ...
    mem["x_nom"][0, 0] = params["phi_0"]
    mem["x_nom"][0, 1] = params["lam_0"]
    mem["x_nom"][0, 2] = params["h_0"]
    ...
    # Baroemtric Altitude by ISA atomsphere
    baro_alt = (params["t0_isa"] / params["l_isa"]) * (
        1.0
        - (baro_p / params["p0_isa"]) ** ((params["l_isa"] * 287.0528) / params["g_e"])
    )
    ...
    for k in range(1, n_epo):
        ...
        f_calib, w_calib = kin.apply_sensor_calibration(f_raw, w_raw, calib_a, calib_g)
        ...
        if is_vib:
            ...
            accel_var_penalty = (
                (params["k_vib_mult"] * params["sigma_accel"]) ** 2
                - params["sigma_accel"] ** 2
            ) * dt
            gyro_var_penalty = (
                (params["k_vib_mult"] * params["sigma_gyro"]) ** 2
                - params["sigma_gyro"] ** 2
            ) * dt
        ...
        lon_deg = (
            float(np.rad2deg(lon)) % 360.0
        )  # Or normalized at [-180, 180], based on grid
        if lon_deg > 180.0:
            lon_deg -= 360.0
        pt = np.array([[alt_km, lat_deg, lon_deg]])

        # Scale Magnemtometer units in micro-Teslas
        m_n = (
            np.array(
        ...
        H_k = meas.calc_h_matrix(r_m, r_n, lat, alt, dcm_b2n, params["r_arm_b"], m_n)
        ...
        pos_gnss = np.array(
            [gnss_lat[k], gnss_lon[k], gnss_alt[k]],
            dtype=np.float64,
        )
        vel_gnss = np.array([gnss_vE[k], gnss_vN[k], gnss_vU[k]], dtype=np.float64)
    ...
    out_path = project_root / "data" / "aligned_data" / "eskf_output_state.npz"
    np.savez_compressed(
        out_path,
        x_nom=mem["x_nom"],
        P_minus=mem["P_"],
