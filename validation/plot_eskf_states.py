...
from source.phase3_navigation.geodesy_math import (
    calc_radii_vec,
    geodetic_to_enu_vec,
    quat2eul_vec,
)
...
def plot_nominal_kinematics(npz_path: Path):
    ...
    for i in range(3):
        ...
        ax.plot(t, pos_vars[i], "k-", lw=1.2, label="nominal")
        ax.fill_between(
            t,
    ...
    for i in range(3):
        ...
        ax.plot(t, v, "k-", lw=1.2, label="nominal")
        ax.fill_between(
            t, v - 3 * sig_v, v + 3 * sig_v, color="r", alpha=0.2, label="3-sigma"
        )
